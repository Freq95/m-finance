...
describe("migrateData", () => {
  it("returns valid v2 data as-is", () => {
    const v2 = { version: 2, data: [validRecord] };
  ...
  it("migrates v1 (economii_investitii) to v2 (economii + investitii)", () => {
  ...
  it("migrates v0 (legacy data array) to v2", () => {
    const legacy = { data: [validRecord] };
...
describe("validateSchema", () => {
  it("returns valid v2 data unchanged", () => {
    const v2 = { version: 2, data: [validRecord] };
  ...
  it("migrates then returns when v1 data fails current schema parse", () => {
    const defaults = createDefaultCategoryAmounts();
    const { economii: _e, investitii: _i, ...rest } = defaults;
    const v1Record = {
      month: "2024-01",
